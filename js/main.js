...
if (storageOrders) { // если данные есть
    ...
    if (ordersList.length > 0) {
        ...
        for (let i = 0; i < ordersList.length; i++) {
            ...
            const prodIndex = PRODUCTS.findIndex( phone => phone.brand + ' ' + phone.model === ordersList[i].name )
            if (prodIndex >= 0) {
                orderedSmartphoneCounter += ordersList[i].number
            } else {
                oldModels.push(ordersList[i].name)
            }
        }
    }
    ...
    if (oldModels.length > 0) {
        oldModels.forEach( name => {
            ordersList = ordersList.filter( phone => phone.name !== name)
        })
    }
